...
mkdir build
cd build
# -DBUILD_ANDROID_SERVICE=ON -DINSTALL_ANDROID_EXAMPLES=ON -DINSTALL_CREATE_DISTRIB=ON -DBUILD_SHARED_LIBS=ON -DOPENCV_EXTRA_MODULES_PATH=../opencv-contrib/modules
cmake -Wno-dev -DBUILD_TESTS=OFF -DBUILD_FAT_JAVA_LIB=ON -DBUILD_PERF_TESTS=OFF -DANDROID=ON  -DBUILD_ANDROID_EXAMPLES=OFF -DANDROID_NATIVE_API_LEVEL=16  -DCMAKE_BUILD_WITH_INSTALL_RPATH=ON -DCMAKE_TOOLCHAIN_FILE=../opencv/platforms/android/android.toolchain.cmake ../opencv
